...
void PostProcessingMaterial::DrawPass(const SceneContext& sceneContext, ID3DX11EffectTechnique* pTechnique, RenderTarget* pDestination)
{
	...
	pDestination->Clear(DirectX::XMFLOAT4{ DirectX::Colors::Purple });
	...
	auto pDevice = sceneContext.d3dContext.pDeviceContext;
	pDevice->IASetInputLayout(m_pDefaultInputLayout);
	pDevice->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY::D3D10_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);

	constexpr UINT stride = sizeof(VertexPosTex);
	constexpr UINT offset = 0;
	pDevice->IASetVertexBuffers(0, 1, &m_pDefaultVertexBuffer, &stride, &offset);

	D3DX11_TECHNIQUE_DESC pTechniqueDesc;
	pTechnique->GetDesc(&pTechniqueDesc);
	for (UINT i{ 0 }; i < pTechniqueDesc.Passes; ++i)
	{
		pTechnique->GetPassByIndex(i)->Apply(0, pDevice);
		pDevice->Draw(m_VertexCount, 0);
	}
}
