...
VS_OUTPUT VS(VS_INPUT input){
	...
	for(int i = 0; i < 4; ++i)
	{
		float boneIdx = input.blendindices[i];

		if(boneIdx >= 0)
		{
			transformedPosition += mul(float4(input.pos,1.0f), gBones[boneIdx]) * input.blendweights[i]; 
			transformedNormal += mul(input.normal, (float3x3)gBones[boneIdx]) * input.blendweights[i]; 
		}
	}
